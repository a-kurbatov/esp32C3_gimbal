...
static int opProcess(sh2_t *pSh2, const sh2_Op_t *pOp)
{
    ...
    while ((pSh2->pOp != 0) &&
        ...
        shtp_service(pSh2->pShtp);
            ...
            // Yield briefly to allow other tasks (and the SPI HAL) to progress and
            // avoid tight busy-wait re-entrancy into the HAL.
    ...
    while (cursor < len) {
        ...
        if (reportLen == 0) {
            ...
            if (reportId == SENSORHUB_BASE_TIMESTAMP_REF) {
                ...
                // Copy bounded by our buffer to avoid overflow if reportLen is larger
                uint8_t copyLen = (reportLen > SH2_MAX_SENSOR_EVENT_LEN) ? SH2_MAX_SENSOR_EVENT_LEN : reportLen;
                memcpy(event.report, pReport, copyLen);
                event.len = copyLen;
...
const sh2_Op_t getSensorConfigOp = {
    .timeout_us = 300000, // 300 ms bounded wait to avoid stalling task/WDT
...
const sh2_Op_t reinitOp = {
    .timeout_us = 500000, // 500 ms timeout to avoid blocking forever
    .start = reinitStart,
    .rx = reinitRx,
};
