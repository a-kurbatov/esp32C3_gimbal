...
static int hal_read(sh2_Hal_t *self, uint8_t *pBuffer, unsigned len, uint32_t *t_us) {
    ...
    if (payload_len) {
        ...
        esp_rom_delay_us(1);
        size_t xfer = payload_len;
        if (xfer > sizeof(s_dummy)) xfer = sizeof(s_dummy);
        spi_transaction_t tp;
        memset(&tp, 0, sizeof(tp));
        // If padding is needed, keep CS active across payload and pad bytes
        tp.flags = (pad_bytes > 0) ? SPI_TRANS_CS_KEEP_ACTIVE : 0;
        tp.length = 8 * xfer;
        tp.rxlength = tp.length;
        tp.tx_buffer = s_dummy;
        tp.rx_buffer = pBuffer + 4;
        e = spi_device_transmit(s_dev, &tp);
        if (e != ESP_OK) { spi_device_release_bus(s_dev); return 0; }
        // Note: assuming payload fits in one transfer (<= SH2_HAL_DMA_SIZE)
